...
combined_data_short = combined_data %>%
  dplyr::select(year, grass, pdsi, spei, summer_pdsi, summer_spei, yearly_ppt_mm, summer_ppt_mm, winter_ppt_mm) %>%
...
combined_data_enso = combined_data %>%
  dplyr::select(year, grass, nino34, enso_phase) %>%
  dplyr::filter(year %in% 1916:1979) 
...
# save plot to file
png('Figures/2023_02/CCF_PDO.png', width=5, height=5, units='in', res=600)
plot.file <- ccf(corr_data$pdo, corr_data$grass, main='PDO')
plot.file
dev.off()

# correlation with 10 year mean
#ccf(corr_data$pdo_mean_10y, corr_data$grass)
```
