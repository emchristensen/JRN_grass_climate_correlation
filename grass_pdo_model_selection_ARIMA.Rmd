...
timeplot = ggplot(modelfit, aes(x=year, y=predicted_grass)) +
  geom_line(aes(color = 'Model \nprediction'), linewidth=1) +
  geom_point(data=model_data, aes(y=grass, color='Grass \nmeasurement'), size=.7) +
  scale_color_manual(name='',values=cbPalette[c(7,3)]) +
  xlab('') +
  ylab('Grass Cover (m2)') +
  theme_bw() +
  theme(axis.text=element_text(size=6), axis.title=element_text(size=8), legend.text=element_text(size=6), legend.title=element_text(size=8), legend.position = 'right', legend.margin=margin(l=-10))
timeplot

#ggsave('Figures/2023_02/grass_pdo10y_timeseries.png', plot=timeplot, width=5, height=3)
ggsave('Figures/2023_04/Fig1.tiff', plot=timeplot, width=8.5, height=5, units='cm', dpi=600)
```
...
grasspred = ggplot(predictionvalues, aes(x=year)) +
  ...
  geom_point(data=combined_data, aes(x=year, y=grass), size=.7) +
  xlim(1916, 2022) +
  theme_bw() +
...
ggsave('Figures/2023_04/Fig2.tiff', plot=grasspred, width=8.5, height=6, units='cm', dpi=600)
