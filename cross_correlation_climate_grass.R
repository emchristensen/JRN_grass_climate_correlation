...
totalgrass = grassts %>%
  ...
  summarize(grass = mean(grass_cover),
            shrub = mean(shrub_cover))
...
pptts = read.csv('data/climate_variables.csv')
...
soi_long = read.csv('data/SOI_long_1866_2020.csv')
...
ppt_ts = ts(data=ppt$yearly_ppt_mm[1:53], frequency=1, start=1916)
grass_ts = ts(data=totalgrass$grass[1:53], frequency=1, start=1916)
pdo_ts = ts(data=pdo_long$mean_pdo[1:53], frequency=1, start=1916)

plot(cbind(pdo_ts, grass_ts))

ccf(ppt_ts, grass_ts, ylab='Cross-correlation')

# looks like some significance in 0 and -1 lag meaning grass is correlated to same year and previous year PDO
...
#cross correlation at 0 and -1 -- meaning grass is correlated to same year and previous year PDO