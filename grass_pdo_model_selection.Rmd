...
model_data = combined_data %>%
  ...
  dplyr::select(year, grass, pdo_mean_10y, enso_phase, PC1, PC2)

# small time scale
model1 = lm(grass ~ PC1 + PC2, data=model_data)
...
model4 = lm(grass ~ pdo_mean_10y + PC1 + PC2, data=model_data)
model5 = lm(grass ~ pdo_mean_10y + enso_phase, data=model_data)
model6 = lm(grass ~ enso_phase + PC1 + PC2, data=model_data)
model7 = lm(grass ~ pdo_mean_10y + enso_phase + PC1 + PC2, data=model_data)
...
modellist = list(modelnull, model1, model2, model3, model4, model5, model6, model7) 
...
There are two almost equivalent models in terms of AICc and rsquared. The simplest of these has only 10-year mean of pdo as a predictor. 
...
ggsave('Figures/2022_09/scatterplot_pdo10y_grass.png', plot=scatterplot, width=4, height=3)
...
ggsave('Figures/2022_09/grass_pdo10y_timeseries.png', plot=timeplot, width=5, height=3)
```
...
grasspred = ggplot(modelprediction, aes(x=year)) +
  ...
  ylab('Grass cover (m^2)')
grasspred
ggsave('Figures/2022_09/grass_prediction_from_PDO.png', plot=grasspred, width=5, height=4)
